              ...
              <div key={date} className="compact-date-group">
                ...
                <div className="compact-table">
                  ...
                  {sortedItems.map((item, idx) => {
                    ...
                    // Форматирование с учётом unit
const { value: factValue } = formatWithUnit(
  item.factQuantity,
  item.unit ?? null,  // ← преобразуем undefined в null
  item.material
);
          ...
          <div key={date} className="compact-date-group">
            ...
            <div className="compact-table">
              ...
              {[...items].sort((a, b) => {
                ...
                // Форматирование с учётом unit (для ТАС unit может быть NULL)
                const { value: factValue } = formatWithUnit(
                  item.factQuantity,
                  item.unit ?? null,
                  item.material
...
//                     const displayFact = Math.round(item.factQuantity);
                    
//                     // Форматирование Заяв (план) - конвертируем кг в тонны и округляем
//                     let displayPlan = item.planQuantity;
//                     // Если план больше 10000 - это килограммы, переводим в тонны
//                     if (displayPlan > 10000) {
...
//                   // Округляем до целых (без деления на 1000)
//                   const displayFact = Math.round(item.factQuantity);
