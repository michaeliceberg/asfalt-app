import { NextResponse } from 'next/server';
import { db, sql } from '@/lib/db';
import { users } from '@/lib/db/schema';
import { eq } from 'drizzle-orm';
import fs from 'fs';
import path from 'path';
import { sendPushNotification } from '@/lib/push-notifications';

type CheckStatus = 'ok' | 'warning' | 'error';

export async function GET() {
  const checks: {
    database: { status: CheckStatus; details: string };
    filesystem: { status: CheckStatus; details: string };
    sync: { status: CheckStatus; details: string };
    memory: { used: number; total: number; percent: number };
  } = {
    database: { status: 'ok', details: '' },
    filesystem: { status: 'ok', details: '' },
    sync: { status: 'ok', details: '' },
    memory: { used: 0, total: 0, percent: 0 },
  };

  const alerts: string[] = [];

  // 1. Проверка базы данных
  try {
    await db.run(sql`SELECT 1`);
    checks.database.status = 'ok';
  } catch (error) {
    checks.database.status = 'error';
    checks.database.details = error instanceof Error ? error.message : 'Unknown error';
    alerts.push('❌ База данных не отвечает!');
  }

  // 2. Проверка файловой системы
  try {
    const testFile = path.join(process.cwd(), 'data', 'test-write.tmp');
    fs.writeFileSync(testFile, 'test');
    fs.unlinkSync(testFile);
    checks.filesystem.status = 'ok';
  } catch (error) {
    checks.filesystem.status = 'error';
    checks.filesystem.details = error instanceof Error ? error.message : 'Unknown error';
    alerts.push('❌ Нет прав на запись в папку data/');
  }

  // 3. Проверка синхронизации
  try {
    const syncFiles = ['last-sync-shipments.json', 'last-sync-requests.json', 'last-sync.json'];
    let syncOk = true;
    for (const file of syncFiles) {
      const filePath = path.join(process.cwd(), 'data', file);
      if (fs.existsSync(filePath)) {
        const data = JSON.parse(fs.readFileSync(filePath, 'utf-8'));
        if (data.lastSync) {
          const lastSyncTime = new Date(data.lastSync).getTime();
          const minutesAgo = Math.floor((Date.now() - lastSyncTime) / 1000 / 60);
          if (minutesAgo > 30) {
            syncOk = false;
            alerts.push(`⚠️ Синхронизация ${file} не обновлялась ${minutesAgo} минут`);
          }
        }
      }
    }
    checks.sync.status = syncOk ? 'ok' : 'warning';
  } catch (error) {
    checks.sync.status = 'warning';
    checks.sync.details = error instanceof Error ? error.message : 'Unknown error';
    alerts.push('⚠️ Ошибка проверки синхронизации');
  }

  // 4. Память
  const memoryUsage = process.memoryUsage();
  checks.memory = {
    used: Math.round(memoryUsage.heapUsed / 1024 / 1024),
    total: Math.round(memoryUsage.heapTotal / 1024 / 1024),
    percent: Math.round((memoryUsage.heapUsed / memoryUsage.heapTotal) * 100),
  };

  if (checks.memory.percent > 90) {
    alerts.push(`⚠️ Высокое использование памяти: ${checks.memory.percent}%`);
  }

  const status = alerts.some(a => a.includes('❌')) ? 'critical' 
    : alerts.length > 0 ? 'warning' 
    : 'healthy';

  // ✅ Отправляем уведомление администратору при проблемах
  if (alerts.length > 0) {
    try {
      // Находим администратора (group_id = 1)
      const adminUsers = await db
        .select()
        .from(users)
        .where(eq(users.group_id, 1))
        .limit(1);
      
      if (adminUsers.length > 0) {
        const adminId = adminUsers[0].id;
        const title = status === 'critical' ? '🚨 КРИТИЧЕСКАЯ ОШИБКА!' : '⚠️ Предупреждение';
        const body = alerts.slice(0, 3).join('\n') + (alerts.length > 3 ? `\n... и ещё ${alerts.length - 3} проблем` : '');
        
        await sendPushNotification(adminId, {
          title,
          body,
          tag: `health-${Date.now()}`,
          url: '/api/health',
        });
        console.log(`✅ Health check уведомление отправлено администратору (ID: ${adminId})`);
      }
    } catch (error) {
      console.error('❌ Ошибка отправки health check уведомления:', error);
    }
  }

  return NextResponse.json({
    status,
    timestamp: new Date().toISOString(),
    checks,
    alerts,
    uptime: Math.floor(process.uptime()),
  });
}





// import { NextResponse } from 'next/server';
// import { db, sql } from '@/lib/db';
// import fs from 'fs';
// import path from 'path';

// type CheckStatus = 'ok' | 'warning' | 'error';

// export async function GET() {
//   const checks: {
//     database: { status: CheckStatus; details: string };
//     filesystem: { status: CheckStatus; details: string };
//     sync: { status: CheckStatus; details: string };
//     memory: { used: number; total: number; percent: number };
//   } = {
//     database: { status: 'ok', details: '' },
//     filesystem: { status: 'ok', details: '' },
//     sync: { status: 'ok', details: '' },
//     memory: { used: 0, total: 0, percent: 0 },
//   };

//   const alerts: string[] = [];

//   // 1. Проверка базы данных
//   try {
//     await db.run(sql`SELECT 1`);
//     checks.database.status = 'ok';
//   } catch (error) {
//     checks.database.status = 'error';
//     checks.database.details = error instanceof Error ? error.message : 'Unknown error';
//     alerts.push('❌ База данных не отвечает!');
//   }

//   // 2. Проверка файловой системы
//   try {
//     const testFile = path.join(process.cwd(), 'data', 'test-write.tmp');
//     fs.writeFileSync(testFile, 'test');
//     fs.unlinkSync(testFile);
//     checks.filesystem.status = 'ok';
//   } catch (error) {
//     checks.filesystem.status = 'error';
//     checks.filesystem.details = error instanceof Error ? error.message : 'Unknown error';
//     alerts.push('❌ Нет прав на запись в папку data/');
//   }

//   // 3. Проверка синхронизации
//   try {
//     const syncFiles = ['last-sync-shipments.json', 'last-sync-requests.json', 'last-sync.json'];
//     let syncOk = true;
//     for (const file of syncFiles) {
//       const filePath = path.join(process.cwd(), 'data', file);
//       if (fs.existsSync(filePath)) {
//         const data = JSON.parse(fs.readFileSync(filePath, 'utf-8'));
//         if (data.lastSync) {
//           const lastSyncTime = new Date(data.lastSync).getTime();
//           const minutesAgo = Math.floor((Date.now() - lastSyncTime) / 1000 / 60);
//           if (minutesAgo > 30) {
//             syncOk = false;
//             alerts.push(`⚠️ Синхронизация ${file} не обновлялась ${minutesAgo} минут`);
//           }
//         }
//       }
//     }
//     checks.sync.status = syncOk ? 'ok' : 'warning';
//   } catch (error) {
//     checks.sync.status = 'warning';
//     checks.sync.details = error instanceof Error ? error.message : 'Unknown error';
//     alerts.push('⚠️ Ошибка проверки синхронизации');
//   }

//   // 4. Память
//   const memoryUsage = process.memoryUsage();
//   checks.memory = {
//     used: Math.round(memoryUsage.heapUsed / 1024 / 1024),
//     total: Math.round(memoryUsage.heapTotal / 1024 / 1024),
//     percent: Math.round((memoryUsage.heapUsed / memoryUsage.heapTotal) * 100),
//   };

//   if (checks.memory.percent > 90) {
//     alerts.push(`⚠️ Высокое использование памяти: ${checks.memory.percent}%`);
//   }

//   const status = alerts.some(a => a.includes('❌')) ? 'critical' 
//     : alerts.length > 0 ? 'warning' 
//     : 'healthy';

//   return NextResponse.json({
//     status,
//     timestamp: new Date().toISOString(),
//     checks,
//     alerts,
//     uptime: Math.floor(process.uptime()),
//   });
// }




// import { NextResponse } from 'next/server';
// import { db, sql } from '@/lib/db';
// import fs from 'fs';
// import path from 'path';

// export async function GET() {
//   const checks = {
//     database: { status: 'ok' as const, details: '' as string },
//     filesystem: { status: 'ok' as const, details: '' as string },
//     sync: { status: 'ok' as const, details: '' as string },
//     memory: { used: 0, total: 0, percent: 0 },
//   };

//   const alerts: string[] = [];

//   // 1. Проверка базы данных
//   try {
//     await db.run(sql`SELECT 1`);
//     checks.database.status = 'ok';
//   } catch (error) {
//     checks.database.status = 'error' as const;
//     checks.database.details = error instanceof Error ? error.message : 'Unknown error';
//     alerts.push('❌ База данных не отвечает!');
//   }

//   // 2. Проверка файловой системы
//   try {
//     const testFile = path.join(process.cwd(), 'data', 'test-write.tmp');
//     fs.writeFileSync(testFile, 'test');
//     fs.unlinkSync(testFile);
//     checks.filesystem.status = 'ok';
//   } catch (error) {
//     checks.filesystem.status = 'error' as const;
//     checks.filesystem.details = error instanceof Error ? error.message : 'Unknown error';
//     alerts.push('❌ Нет прав на запись в папку data/');
//   }

//   // 3. Проверка синхронизации
//   try {
//     const syncFiles = ['last-sync-shipments.json', 'last-sync-requests.json', 'last-sync.json'];
//     let syncOk = true;
//     for (const file of syncFiles) {
//       const filePath = path.join(process.cwd(), 'data', file);
//       if (fs.existsSync(filePath)) {
//         const data = JSON.parse(fs.readFileSync(filePath, 'utf-8'));
//         if (data.lastSync) {
//           const lastSyncTime = new Date(data.lastSync).getTime();
//           const minutesAgo = Math.floor((Date.now() - lastSyncTime) / 1000 / 60);
//           if (minutesAgo > 30) {
//             syncOk = false;
//             alerts.push(`⚠️ Синхронизация ${file} не обновлялась ${minutesAgo} минут`);
//           }
//         }
//       }
//     }
//     checks.sync.status = syncOk ? 'ok' : ('warning' as const);
//   } catch (error) {
//     checks.sync.status = 'warning' as const;
//     checks.sync.details = error instanceof Error ? error.message : 'Unknown error';
//     alerts.push('⚠️ Ошибка проверки синхронизации');
//   }

//   // 4. Память
//   const memoryUsage = process.memoryUsage();
//   checks.memory = {
//     used: Math.round(memoryUsage.heapUsed / 1024 / 1024),
//     total: Math.round(memoryUsage.heapTotal / 1024 / 1024),
//     percent: Math.round((memoryUsage.heapUsed / memoryUsage.heapTotal) * 100),
//   };

//   if (checks.memory.percent > 90) {
//     alerts.push(`⚠️ Высокое использование памяти: ${checks.memory.percent}%`);
//   }

//   const status = alerts.some(a => a.includes('❌')) ? 'critical' 
//     : alerts.length > 0 ? 'warning' 
//     : 'healthy';

//   return NextResponse.json({
//     status,
//     timestamp: new Date().toISOString(),
//     checks,
//     alerts,
//     uptime: Math.floor(process.uptime()),
//   });
// }
