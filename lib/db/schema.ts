...
export const shipments = sqliteTable('shipments', {
  ...
  // Минимальное расстояние до пункта назначения, зафиксированное за весь
  // рейс (scripts/calc-distances.ts обновляет на каждом прогоне, если
  // текущее расстояние меньше). Раньше arrived выставлялся только по
  // прямому попаданию в радиус ARRIVAL_THRESHOLD_KM ПРЯМО СЕЙЧАС — если
  // машина заехала и уехала между двумя пятиминутными прогонами крона
  // (или координата точки назначения неточная), этот момент можно было
  // пропустить навсегда, и расстояние потом считалось от текущей позиции
  // машины (которая могла уехать куда угодно) до старой точки — цифра
  // выглядела как живой прогресс, а была бессмыслицей. min_distance_to_dest
  // даёт запасной сигнал: "подъезжал близко, а потом расстояние выросло" =
  // скорее всего доставил и уехал, даже если порог arrival не сработал.
  min_distance_to_dest: real('min_distance_to_dest'),
});
