...
// Латинские буквы-омографы → кириллические аналоги. Госномер иногда
// вводят с английской раскладкой (буквы выглядят одинаково, но это
// разные символы), из-за чего сравнение строк "К118РУ790" !== "K118PY790"
// молча проваливается.
const LATIN_TO_CYRILLIC_PLATE_MAP: Record<string, string> = {
  A: 'А', B: 'В', C: 'С', E: 'Е', H: 'Н', K: 'К',
  M: 'М', O: 'О', P: 'Р', T: 'Т', X: 'Х', Y: 'У',
};

/**
 * Приводит госномер к единому виду для сопоставления между разными
 * источниками данных (1С, GPS-платформа, наш список машин в lib/trucks.ts):
 * убирает пробелы (по краям и внутри — 1С иногда присылает госномер с
 * хвостовым пробелом), переводит в верхний регистр, заменяет
 * латиницу-омографы на кириллицу.
 */
export function normalizePlate(plate: string | null | undefined): string {
  if (!plate) return '';
  const upper = plate.toUpperCase().replace(/\s+/g, '');
  return upper.replace(/[ABCEHKMOPTXY]/g, (ch) => LATIN_TO_CYRILLIC_PLATE_MAP[ch] || ch);
}
