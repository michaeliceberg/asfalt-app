...
class TelegramPollingService {
    ...
    private async getPlanWithFact(dateStr: string, title: string): Promise<string> {
        ...
        for (const division of sortedDivisions) {
            ...
            message += `🏭 ${divisionName} ${divisionEmoji} ${divisionFactTotal}/${divisionPlanTotal} т\n`;
            ...
            for (const item of items) {
                ...
                const percent = consigneePlanTotal > 0 ? (consigneeFactTotal / consigneePlanTotal) * 100 : 0;
                const emoji = this.getProgressEmoji(percent);
                
                if (consigneePlanTotal > 0) {
                    message += `   ${emoji} *${item.consignee}* ${consigneeFactTotal}/${consigneePlanTotal} т\n`;
                } else {
                    message += `   🟠 *${item.consignee}* ${consigneeFactTotal} т (без плана)\n`;
                }
                
                // Показываем первый материал (как в приложении)
